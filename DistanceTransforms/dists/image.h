      ...
      for(int j=-1; j<2; j++){
        for(int k=-1; k<2; k++){
          if(valid(x+i,y+j,z+k) && !(i==0 && j==0 && k==0)){
            ...
            addto.push_back(abs(i)+abs(j)+abs(k));   /* TODO: change the indexing order depending on PY?*/
          }
        }
      }
    }
    ...
    for(int i=-1; i<2; i++){
      for(int j=-1; j<2; j++){
        if(valid(x+i,y+j) && !(i==0 && j==0)){
          ...
          addto.push_back(abs(i)+abs(j));   /* TODO: change the indexing order depending on PY?*/
        }
      }
    }
  }
