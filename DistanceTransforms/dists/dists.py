...
c_distlib.MBD.argtypes = [ctl.ndpointer(ctypes.c_uint8, flags='aligned, contiguous'), 
...
c_distlib.GEO.argtypes = [ctl.ndpointer(ctypes.c_uint8, flags='aligned, contiguous'), 
                            ctl.ndpointer(ctypes.c_uint8, flags='aligned, contiguous'),
                            ctypes.c_int, ctypes.c_int, ctypes.c_int,
                            ctl.ndpointer(ctypes.c_double, flags='aligned, contiguous, writeable')]
...
def getMBD(image: np.ndarray, label:np.ndarray) -> np.ndarray:  #np.ndarray[Any, np.dtype[np.int32]]) -> np.ndarray[Any, np.dtype[np.double]]
    ...
    for i in range(classes):
        c_distlib.MBD(label, image, sizex, sizey, sizez, dt[i])

    return np.stack(dt, axis=-1) #here and above I assume that input label image has classes as last dim. 

def getGEO(image: np.ndarray, label:np.ndarray) -> np.ndarray:  #np.ndarray[Any, np.dtype[np.int32]]) -> np.ndarray[Any, np.dtype[np.double]]
    image = np.ascontiguousarray(image, dtype=np.uint8)
    label = np.ascontiguousarray(label, dtype=np.uint8)
    image.setflags(align=True)
    label.setflags(align=True)
    ...
    dt = [np.zeros((sizex, sizey, sizez), dtype=np.double) for i in range(classes)]
    for i in range(classes):
        c_distlib.GEO(label, image, sizex, sizey, sizez, dt[i])
